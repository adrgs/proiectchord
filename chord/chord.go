...
type Node struct {
	...
	Peers map[string]pb.HelloServiceClient
}

func (node *Node) SayHello(ctx context.Context, stream *pb.HelloRequest) (*pb.HelloReply, error) {
	return &pb.HelloReply{Message: "Hello from " + node.Name}, nil
}
...
func (node *Node) StartListening() {
	...
	pb.RegisterHelloServiceServer(grpcServer, node)
...
func (node *Node) Start() error {
	node.Peers = make(map[string]pb.HelloServiceClient)
...
func (node *Node) SetupClient(name string, addr string) {
	...
	node.Peers[name] = pb.NewHelloServiceClient(conn)

	response, err := node.Peers[name].SayHello(context.Background(), &pb.HelloRequest{Name: node.Name})
	if err != nil {
